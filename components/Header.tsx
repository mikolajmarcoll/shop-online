...
export const Header = () => {
  ...
  const getLinkClassName = (linkPathname: String) => {
    const selectedClassName = 'text-gray-800'
    const inactiveClassName = 'text-gray-500 transition hover:text-gray-500/75'

    return `p-4 ${
      pathname === linkPathname ? selectedClassName : inactiveClassName
    }`
  }
            ...
            <nav aria-label='Site Nav' className='hidden md:block'>
              <ul className='flex items-center gap-6 text-sm'>
                {navigationRoutes.map(({ name, path }) => (
                  <li key={`navigation-option-${name}`}>
                    <Link href={path} className={getLinkClassName(path)}>
                      {name}
                    </Link>
                  </li>
                ))}
